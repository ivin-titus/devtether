...
name: ponytail
description: The "Lazy Senior Dev" mindset. Enforces YAGNI, minimal diffs, idiomatic Go, and standard library over external dependencies.
...
Example: the user asks for a simple HTTP request with a timeout. Instead of installing a third-party `requests` library or building a massive `HttpClient` interface factory, write:

```go
client := &http.Client{Timeout: 5 * time.Second}
```

Example 2: the user asks to reverse a slice or sort it. Instead of importing `github.com/some/random-utils` or writing a custom quicksort, write:

```go
slices.Reverse(mySlice)
slices.Sort(mySlice)
```
Relying on the Go 1.27.1 standard library is non-negotiable before reaching for third-party dependencies.
