...
3. **Secure Root Daemonization:** When running as a system daemon or binding to privileged ports via `sudo` (e.g., macOS port 80), the daemon strictly isolates its runtime directory (e.g., `/tmp/devtether-0` with `0700` permissions, owned by UID 0). This prevents root-owned sockets from leaking into unprivileged user directories and enables safe mixed-privilege operations.
